...
// @desc    Cập nhật user profile (Tên, Email, SĐT, Địa chỉ)
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    // Kiểm tra email đã được sử dụng chưa (nếu có thay đổi email)
    if (email) {
      const existingUser = await User.findOne({ 
        email: email.toLowerCase(),
        walletAddress: { $ne: req.user.walletAddress } // Loại trừ chính mình
      });
